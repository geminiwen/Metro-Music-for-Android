...
public class LrcUtil {
	...
	public List<SentenceModel> parseLrc(String line,Song song)
	{
		...
		// 结尾为真正第一句歌词的开始
		if (list.size() == 0) {
			list.add(new SentenceModel(song.getTitle(), 0,
								Integer.MAX_VALUE));
			return list;
		} else {
			...
			list.add(
					0,
					new SentenceModel(song.getTitle(), 0, first
									.getFromTime()));
			}
			int size = list.size();
			...
			if (list.size() == 1) {
				...
				last.setToTime(song == null ? Integer.MAX_VALUE : song
						.getLength() * 1000 + 1000);	
				}
		return list;
	}
